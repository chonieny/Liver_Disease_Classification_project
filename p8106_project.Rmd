...
model.rf <- train(outcome~., data = liver_df1, subset = rowTrain, method = "rpart", 
                  trControl = train_control, parms = list(split = "gini"))
RF.pred <- predict(model.rf, newdata = liver_df1[-rowTrain, ])
confu_dtree <- confusionMatrix(RF.pred, liver_df1[-rowTrain, ]$outcome)
confu_dtree

#AUC
PredDTModel <- predict(model.rf, newdata = liver_df1[-rowTrain, ], type = 'prob')
DTPrediction <- prediction(PredDTModel[2],liver_df1[-rowTrain, ]$outcome)
...
res <- resamples(list(MARS = model.mars, 
                      ...
                      GBM = gbmA.model,
                      NB = model.nb)) 
summary(res) #RF, SVM not included
