...
# RANDOM FORESTS
```{r}
ctrl3 = trainControl(method = "cv", classProbs = T, summaryFunction = twoClassSummary)
...
model.rf = train(outcome ~ ., liver_df1, subset = rowTrain, method = "ranger", 
               tuneGrid = rf.grid, metric = "ROC", trControl = ctrl3)
...
svm.radial.model <- train(outcome ~., 
                          ...
                          tuneGrid = svmr.grid,
                          prob.model = TRUE,
                          verbose=T) # not working, saying "maximum number of iterations reached"
...
SVM.pred = predict(svm.radial.model, newdata = liver_df1[-rowTrain, ], type = "prob")[,1] #ERROR
...
#pred6 = prediction(as.numeric(SVM.pred), liver_df1$outcome[-rowTrain], ) #ERROR
